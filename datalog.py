...
class DataLogIterator:
    ...
    def __next__(self) -> DataLogRecord:
        if len(self.buf) < (self.pos + 4):
            raise StopIteration
        entryLen = (self.buf[self.pos] & 0x3) + 1               #length of smallest int the number can be stored in
        sizeLen = ((self.buf[self.pos] >> 2) & 0x3) + 1         #if can be stored in 8 bit the number is 1, if 16 bit the number is 2
        timestampLen = ((self.buf[self.pos] >> 4) & 0x7) + 1    #and so on
...
class DataLogReader:
    ...
    def __iter__(self) -> DataLogIterator:
        #creates an iterator for the data log that starts at the first record

...
def Main(planner:str):
    ...
    template = {"array_names": [],"trace_names": [],"sources": [],"blacklisted_types": []}
    group_data = {}
    groups = list(group_data.keys())

    settings = {"auto_group_by_can": False,"auto_group_by_array": False,"blacklist_NT": False,"global_blacklist": []}
    ...
    layout = [
                ...
                [gui.Listbox(values=groups, size=(30, 3), key='-LIST-', auto_size_text=True, expand_x=True, expand_y=True, background_color='black', text_color='white')],
    ...
    def edit_window(name, group_data):
        ...
        window2 = gui.Window(f'Editing {name}', layout=layout, finalize=True, resizable=True, size=(1280, 720))
        def __update_lists():
            window2['selcted_names'].update(values=group_data[name]['trace_names'])
            window2['selcted_arrays'].update(values=group_data[name]['array_names'])
            window2['selcted_sources'].update(values=group_data[name]['sources'])
        ...
        while True:
            ...
            if values['selcted_arrays']:
                idx = find(values['selcted_arrays'][0], group_data[name]['array_names'])
                group_data[name]['array_names'].pop(idx)
            if values['selcted_sources']:
                idx = find(values['selcted_sources'][0], group_data[name]['sources'])
                group_data[name]['sources'].pop(idx)
            if values['selcted_types']:
                idx = find(values['selcted_types'][0], group_data[name]['blacklisted_types'])
    ...
    def setting_window(settings):
        layout = [
            ...
            [gui.Radio('auto group by can ID', 'can', key='can', default=settings['auto_group_by_can'], enable_events=True), 
                gui.Radio('Auto group by array name', 'array', key='array', default=settings['auto_group_by_array'], enable_events=True),
            ...
            [gui.Listbox(values=settings['global_blacklist'], size=(30, 5), key='global_blacklist', auto_size_text=True, expand_x=True, expand_y=True, enable_events=True),
                gui.Listbox(values=jdata['names'], size=(30, 5), key='possible_names', auto_size_text=True, expand_x=True, expand_y=True, enable_events=True)],
            [gui.Text('Robot Name', justification='left', size=(30, 1)), gui.Input(key='robot_name', size=(30, 1), enable_events=True)],
        ]
        window3 = gui.Window('Settings', layout=layout, finalize=True, resizable=True, size=(1280, 720))
        def __update_lists():
            window3['global_blacklist'].update(values=settings['global_blacklist'])
        while True:
            __update_lists()
            ...
            if values['possible_names'] and event == 'possible_names':
                if values['possible_names'][0] not in settings['global_blacklist']:
                    settings['global_blacklist'].append(values['possible_names'][0])
            if event == 'can':
                settings['auto_group_by_can'] = not settings['auto_group_by_can']
                window3.element_list()[1].update(value=settings['auto_group_by_can'])
            if event == 'array':
                settings['auto_group_by_array'] = not settings['auto_group_by_array']
                window3.element_list()[2].update(value=settings['auto_group_by_array'])
            if event == 'nt':
                settings['blacklist_NT'] = not settings['blacklist_NT']
                window3.element_list()[3].update(value=settings['blacklist_NT'])
